...
from clustercontrast.evaluators import build_test_transform, evaluate_llcm_mode, extract_features
...
_PARSER_COMMON_DEFAULTS = {
    ...
    "logs_dir_default": osp.join(_WORKING_DIR, "logs_llcm"),
    ...
    "k1_default": 25,
    "seed_default": 1,
    "log_s1_default": "llcm_s1/dmdl",
    "log_s2_default": "llcm_s2/dmdl",
}
...
def run_stage1(args=None):
    ...
    dataset_ir = get_data('llcm_ir', args.data_dir)
    dataset_rgb = get_data('llcm_rgb', args.data_dir)
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            features_rgb, _, cids_rgb = extract_features(
                model, cluster_loader_rgb, print_freq=50, mode=1, runtime_args=args
            )
            features_rgb = torch.cat([features_rgb[f].unsqueeze(0) for f, _, _ in sorted_rgb_train], 0).cuda()
            cids_rgb = np.array([cids_rgb[f].item() for f, _, _ in sorted_rgb_train])

            time.sleep(5)
            print('==> Create pseudo labels for unlabeled IR data')
            features_ir, _, cids_ir = extract_features(
                model, cluster_loader_ir, print_freq=50, mode=2, runtime_args=args
            )
            time.sleep(5)
            features_ir = torch.cat([features_ir[f].unsqueeze(0) for f, _, _ in sorted_ir_train], 0).cuda()
            cids_ir = np.array([cids_ir[f].item() for f, _, _ in sorted_ir_train])
...
##############################
            ...
            mAP = evaluate_llcm_mode(model, args, transform_test, mode=1)
#################################
...
############################
    ...
    transform_test = build_test_transform(args)
    evaluate_llcm_mode(model, args, transform_test, mode=1)
    ...
    evaluate_llcm_mode(model, args, transform_test, mode=2)
    end_time = time.monotonic()
    print('Total running time: ', timedelta(seconds=end_time - start_time))
...
def run_stage2(args=None):
    ...
    iters = args.iters if (args.iters > 0) else None
    print("==> Load unlabeled dataset")
    dataset_ir = _stage2_get_data('llcm_ir', args.data_dir)
    dataset_rgb = _stage2_get_data('llcm_rgb', args.data_dir)
    sorted_ir_train = sorted(dataset_ir.train)
    sorted_rgb_train = sorted(dataset_rgb.train)
...
def build_parser(stage=1):
    ...
    return require_dataset_data_dir(parser, "LLCM")
...
def main(args=None):
    ...
    if not getattr(args, 'data_dir', None):
        raise ValueError('LLCM training requires --data-dir to be provided explicitly.')
