...
_GPU_RESOURCES = {}
_FAISS_WARMUP_CACHE = set()


def _gpu_thread_key():
    return (torch.cuda.current_device(), threading.get_ident())


def _get_gpu_resources():
    cache_key = _gpu_thread_key()
    res = _GPU_RESOURCES.get(cache_key)
    if res is None:
        res = faiss.StandardGpuResources()
        res.setDefaultNullStreamAllDevices()
        _GPU_RESOURCES[cache_key] = res
    return res


def _warmup_search_raw_array(feat_dim):
    if not torch.cuda.is_available():
        return

    cache_key = _gpu_thread_key() + (feat_dim,)
    if cache_key in _FAISS_WARMUP_CACHE:
        return

    dummy = F.normalize(torch.randn(32, feat_dim, device=torch.cuda.current_device()), dim=1)
    res = _get_gpu_resources()
    search_raw_array_pytorch(res, dummy, dummy, min(5, dummy.size(0)))
    torch.cuda.synchronize()
    _FAISS_WARMUP_CACHE.add(cache_key)
...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=3, use_float16=False):
    ...
    if (search_option==0):
        ...
        _warmup_search_raw_array(target_features.size(-1))
        res = _get_gpu_resources()
