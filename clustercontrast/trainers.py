...
class CausalTrainer_s1_woCAM(object):
    def __init__(self, encoder, memory=None):
        super(CausalTrainer_s1_woCAM, self).__init__()
        self.encoder = encoder
        self.memory_ir = memory
        self.memory_rgb = memory
        self.tri = OriTripletLoss()

    def train(self, args, epoch, data_loader_ir, data_loader_rgb, optimizer, print_freq=10, train_iters=400, miu=0.5):
        self.encoder.train()
        ...
        for i in range(train_iters):
            with amp.autocast():
                ...
                targets_onehot_noise_ir = F.one_hot(labels_ir, self.memory_ir.num_samples).float().cuda()
                prob_ir = self.memory_ir.get_predict(f_out_ir).detach()
                targets_corrected_ir = comb(
                    torch.cat((prob_ir[split_ir:], prob_ir[:split_ir]), dim=0),
                    targets_onehot_noise_ir,
                    clabels_ir.unsqueeze(1),
                )
                targets_onehot_noise_rgb = F.one_hot(labels_rgb, self.memory_rgb.num_samples).float().cuda()
                prob_rgb = self.memory_rgb.get_predict(f_out_rgb).detach()
                ...
                loss_ir = self.memory_ir(f_out_ir, labels_ir, targets_corrected_ir)
                loss_rgb = self.memory_rgb(f_out_rgb, labels_rgb, targets_corrected_rgb)

                loss_tri_ir, _ = self.tri(f_out_ir, labels_ir)
                loss_tri_rgb, _ = self.tri(f_out_rgb, labels_rgb)

                cross_loss = f_out_ir.new_tensor(0.0)
                ...
                loss = loss_ir + loss_rgb + miu * cross_loss + sigma * loss_tri + loss_try
...
class CausalTrainer_s2_woCAM(object):
    def __init__(self, encoder, memory=None):
        super(CausalTrainer_s2_woCAM, self).__init__()
        self.encoder = encoder
...
class CausalTrainer_s2_woCAM_inter(object):
    def __init__(self, encoder, memory=None):
        super(CausalTrainer_s2_woCAM_inter, self).__init__()
        self.encoder = encoder
        self.memory_all = memory
        self.tri = OriTripletLoss()

    def train(self, args, epoch, data_loader, optimizer, print_freq=10, train_iters=400, miu=0.5):
        self.encoder.train()
        batch_time = AverageMeter()
        data_time = AverageMeter()
        losses = AverageMeter()
