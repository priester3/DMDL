...
class ClusterMemory(nn.Module, ABC):
    ...
    def forward(self, inputs, targets, corrected_targets, mode='rgb', i2r=None, reduction='mean'):
        ...
        logits_rgb_all = torch.zeros(inputs.size(0), self.num_all).to(logits_rgb.dtype).cuda()
        logits_ir_all = torch.zeros(inputs.size(0), self.num_all).to(logits_ir.dtype).cuda()
        ...
        loss_y = F.nll_loss(torch.log(logits_all), targets)
