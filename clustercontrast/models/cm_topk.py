...
class ClusterMemory(nn.Module, ABC):
    def __init__(self, num_features, num_samples, topk=10, temp=0.05, momentum=0.2, use_hard=False):
    ...
    def get_predict(self, inputs):
        ...
        topk_vals, topk_idx = torch.topk(outputs, self.topk, dim=1)
        masked_outputs = torch.full_like(outputs, float('-inf'))
        masked_outputs.scatter_(1, topk_idx, topk_vals)
        return F.softmax(masked_outputs.detach(), dim=1)
