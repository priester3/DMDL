...
def add_conv1(state_dict):
  """Remove the fc layer parameters from state_dict."""
  # for key, value in state_dict.items():
  for key, value in list(state_dict.items()):
    if key.startswith('conv1'):
      state_dict[key] = torch.cat((state_dict[key],state_dict[key][:,2,:,:].unsqueeze(1)),1)
  return state_dict

def remove_conv1(state_dict):
  """Remove the fc layer parameters from state_dict."""
  # for key, value in state_dict.items():
  for key, value in list(state_dict.items()):
    if key.startswith('conv1'):
      state_dict[key] = state_dict[key][:,:3,:,:]
  return state_dict
...
def resnet50_3c(pretrained=False, **kwargs):
