...
model_urls = {
    'ibn_resnet50a': 'resnet50_ibn_a',
    'ibn_resnet101a': 'resnet101_ibn_a',
}


def load_hub_pretrained(model_name):
    hub_model = torch.hub.load('XingangPan/IBN-Net', model_name, pretrained=True)
...
def resnet50_ibn_a(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = load_hub_pretrained(model_urls['ibn_resnet50a'])
...
def resnet101_ibn_a(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = load_hub_pretrained(model_urls['ibn_resnet101a'])
        state_dict = remove_module_key(state_dict)
        model.load_state_dict(state_dict)
    return model
