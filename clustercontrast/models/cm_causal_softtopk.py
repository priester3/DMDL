...
class ClusterMemory(nn.Module, ABC):
    def __init__(self, num_features, num_samples_rgb, num_samples_ir, num_all, topk=10, temp=0.05, momentum=0.2, pro_rgb=None, pro_ir=None):
    ...
    def forward(self, inputs, targets, corrected_targets, mode='rgb', i2r=None, reduction='mean'):
        ...
        rgbpid2new = dict(zip([id.item() for id in self.pids_rgb.clone()],range(self.num_samples_rgb)))
        irpid2new = dict(zip([id.item() for id in self.pids_ir.clone()],range(self.num_samples_ir)))
        if mode == 'rgb':
            targets_new = torch.tensor(
                [rgbpid2new[id.item()] for id in targets.clone()],
                dtype=torch.long,
                device=device,
            )
            ...
            logits_ir /= self.temp
            loss_yc = F.cross_entropy(logits_rgb, targets_new, reduction=reduction)
        else:
            targets_new = torch.tensor(
                [irpid2new[id.item()] for id in targets.clone()],
                dtype=torch.long,
                device=device,
            )
            if i2r:
            ...
            logits_ir = cm(inputs, targets, self.features_ir, self.pids_ir, self.momentum)
            logits_ir /= self.temp
            loss_yc = F.cross_entropy(logits_ir, targets_new, reduction=reduction)
        logits_rgb = softmax(logits_rgb)
        logits_ir = softmax(logits_ir)
        
        logits_rgb_all = torch.zeros(inputs.size(0), self.num_all, dtype=logits_rgb.dtype, device=device)
        logits_ir_all = torch.zeros(inputs.size(0), self.num_all, dtype=logits_ir.dtype, device=device)
    ...
    def get_predict(self, inputs, i2r=None):
        ...
        effective_topk = min(self.topk, outputs.size(1))
        if effective_topk <= 0:
            raise ValueError("softtopk requires at least one cluster feature")
        topk_vals, topk_idx = torch.topk(outputs, effective_topk, dim=1)
        masked_outputs = torch.full_like(outputs, float('-inf'))
        masked_outputs.scatter_(1, topk_idx, topk_vals)
        
        outputs = F.softmax(masked_outputs.detach(), dim=1)
